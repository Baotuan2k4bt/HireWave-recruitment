    ...
    >
      <Combobox.Target>
        <InputBase
          ...
          size="md"
          classNames={{
            input: 'border-slate-200 hover:border-oceanTeal-400 transition-colors',
            label: 'text-sm font-medium text-slate-700 mb-1',
          }}
        />
      </Combobox.Target>

      <Combobox.Dropdown className="rounded-md shadow-md border border-slate-100">
        <Combobox.Options>
          <ScrollArea.Autosize mah={200} type="scroll">
            {options}
            {!exactOptionMatch && search.trim().length > 0 && (
              <Combobox.Option value="$create" className="text-oceanTeal-500">
                + Thêm lựa chọn "{search}"
              </Combobox.Option>
            )}
          </ScrollArea.Autosize>
        </Combobox.Options>
