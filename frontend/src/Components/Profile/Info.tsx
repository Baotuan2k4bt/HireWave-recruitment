...
const Info = () => {
    ...
    return <><div data-aos="zoom-out" className="text-3xl xs-mx:text-2xl font-semibold flex justify-between">{user.name} <div>
        ...
        <ActionIcon onClick={handleClick} variant="subtle" color={edit ? "red.8" : "oceanTeal.4"} size={matches?"md":"lg"} >
    ...
    </div>
        {
            ...
            <div data-aos="zoom-out" className=" flex gap-10 md-mx:gap-5 [&>*]:w-1/2 xs-mx:[&>*]:w-full xs-mx:flex-wrap my-3" >
                ...
                <NumberInput data-aos="zoom-out" label="Kinh nghiệm (năm)" withAsterisk hideControls  clampBehavior="strict" min={1} max={50} {...form.getInputProps('totalExp')} />
            </div>
            </> :
                <>
                    ...
                    <div className="text-lg xs-mx:text-base flex gap-1 items-center text-deepSlate-300">
                        <IconMapPin className="h-5 w-5" stroke={1.5} /> {profile.location}
                    </div>
                    <div className="text-lg xs-mx:text-base flex gap-1 items-center text-deepSlate-300">
                        <IconBriefcase className="h-5 w-5" stroke={1.5} />Kinh nghiệm: {profile.totalExp} năm
                    </div>
                    </>
